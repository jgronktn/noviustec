...
- Node.js + Fastify backend at `api.noviustec.com` (nginx + TLS + systemd)
- Static frontend at `app.noviustec.com` (nginx)
- Postmark Inbound Parse → `/webhooks/postmark-inbound`
...
Browser → app.noviustec.com (frontend, static HTML/JS)
↓ fetch (CORS)
→ api.noviustec.com (nginx + TLS)
↓ proxy
→ Node :3000 (Fastify, systemd service noviustec-api)
↓
→ Files on disk, Anthropic API, Postmark webhooks
Postmark Inbound Parse
↓ POST
→ api.noviustec.com/webhooks/postmark-inbound
...
- **Process management**: systemd (service: `noviustec-api`)
...
# Tail server logs
ssh noviustec@api.noviustec.com 'sudo journalctl -u noviustec-api -f'
