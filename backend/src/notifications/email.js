...
//   POSTMARK_FROM  — sender address, e.g. notifications@noviustec.com.
...
function buildSuccessBody({ proposal }) {
  ...
  const text =
    ...
    `  Reference:  ${refLine}\n` +
    `  Category:   ${category}\n\n` +
    `This entry is now in your inbox awaiting your review at app.noviustec.com.` +
    SIGNATURE_TEXT;
  ...
  const html = wrapHtml(
    `<p>Got your email — here's what we extracted:</p>` +
      ...
      `</table>` +
      `<p>This entry is now in your inbox awaiting your review at <a href="https://app.noviustec.com">app.noviustec.com</a>.</p>`,
  );
...
function buildLowConfidenceBody({ proposal }) {
  ...
  const text =
    ...
    `  Vendor:  ${vendor}\n` +
    `  Total:   ${total}\n\n` +
    `Review it at app.noviustec.com.` +
    SIGNATURE_TEXT;
  ...
  const html = wrapHtml(
    `<p>Got your email. I extracted some data but the <strong>confidence is low</strong> — please double-check before approving:</p>` +
      ...
      `<p>Review it at <a href="https://app.noviustec.com">app.noviustec.com</a>.</p>`,
  );
...
function buildCloudLinkBody({ result }) {
  ...
  const text =
    ...
    `Please download the file and reply with it as a direct attachment, or upload it at app.noviustec.com.${linksText}` +
    SIGNATURE_TEXT;

  const html = wrapHtml(
    `<p>Got your email, but it points at cloud-hosted files (Drive / Dropbox / OneDrive etc.) — I can't follow those automatically.</p>` +
      `<p>Please download the file and reply with it as a direct attachment, or upload it at <a href="https://app.noviustec.com">app.noviustec.com</a>.</p>` +
      linksHtml,
  );
...
function buildOversizeBody({ result }) {
  ...
  const text =
    ...
    `Try compressing the file or uploading directly at app.noviustec.com.` +
    SIGNATURE_TEXT;
  ...
  const html = wrapHtml(
    `<p>Got your email, but the attachment(s) are too large for me to process.</p>` +
      (oversize.length > 0
        ...
        : "") +
      `<p>Try compressing the file or uploading directly at <a href="https://app.noviustec.com">app.noviustec.com</a>.</p>`,
  );
...
function buildUnsupportedBody({ result }) {
  const text =
    ...
    `If this is a receipt, please re-send as one of those formats or upload at app.noviustec.com.` +
    SIGNATURE_TEXT;

  const html = wrapHtml(
    `<p>Got your email, but I couldn't read the attachment format. I currently handle <strong>PDF, JPG, PNG, WEBP, GIF</strong>.</p>` +
      `<p>If this is a receipt, please re-send as one of those formats or upload at <a href="https://app.noviustec.com">app.noviustec.com</a>.</p>`,
  );
...
function buildNotAReceiptBody({ result }) {
  ...
  const text =
    ...
    `If this should be tracked anyway, you can upload it manually at app.noviustec.com.` +
    SIGNATURE_TEXT;
  ...
  const html = wrapHtml(
    `<p>Got your email, but I couldn't identify it as a receipt or invoice. Nothing has been added to your books.</p>` +
      ...
      `<p>If this should be tracked anyway, you can upload it manually at <a href="https://app.noviustec.com">app.noviustec.com</a>.</p>`,
  );
...
function buildGenericFailureBody({ status, reason }) {
  const text =
    ...
    `You can upload it manually at app.noviustec.com to track it anyway.` +
    SIGNATURE_TEXT;
  ...
  const html = wrapHtml(
    `<p>Got your email, but I couldn't parse the financial details (status: <code>${escapeHtml(status)}</code>${reason ? `, reason: <code>${escapeHtml(reason)}</code>` : ""}).</p>` +
      `<p>You can upload it manually at <a href="https://app.noviustec.com">app.noviustec.com</a> to track it anyway.</p>`,
  );
